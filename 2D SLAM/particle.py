...
class particle(object):
    ...
    def update_map(self, MAP, head, neck, dist, ps, roll, pitch):
        ...
        # # Plot the scan
        # img = np.zeros(MAP['map'].shape,dtype=np.uint8)
        # temp = mask < 0
        # img[temp] = 255
    ...
    def get_corr(self, MAP, head, neck, dist, ps, roll, pitch):
        ...
        # Simple corellation method. Just count the number of indices that coincide with occupied indices (sum)
        xis = xis[indGood]
        yis = yis[indGood]
        # c = np.sum(MAP['map'][yis, xis]) / 100
        cmax = 0.
        xreq = 0.
        yreq = 0.
